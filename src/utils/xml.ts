...
import { XMLParser, XMLBuilder, XMLValidator, XmlBuilderOptions, X2jOptions } from 'fast-xml-parser';
import { SchemaDocument, SchemaElement, SchemaComplexType } from '../types/schema.js';

// 기본 XML 파싱 옵션
const DEFAULT_PARSING_OPTIONS: X2jOptions = {
...
// 기본 XML 빌드 옵션
const DEFAULT_BUILDING_OPTIONS: XmlBuilderOptions = {
  ignoreAttributes: false,
  attributeNamePrefix: '@_',
  textNodeName: '#text',
  cdataPropName: '#cdata',
 ...
 * @returns 파싱된 객체
 */
export function parseXml(xmlString: string, options?: Partial<X2jOptions>): any {
 ...
 * @returns XML 문자열
 */
export function buildXml(obj: any, options?: Partial<XmlBuilderOptions>): string {
 ...
 * @returns 파싱된 스키마 문서
 */
export function parseXsdSchema(xsdContent: string, fileName: string): SchemaDocument {
 ...
 * 스키마 노드 찾기
 */
function findSchemaNode(parsedXsd: any): any {
  // 다양한 루트 구조 처리
  if (parsedXsd.schema) return parsedXsd.schema;
  if (parsedXsd['xs:schema']) return parsedXsd['xs:schema'];
  if (parsedXsd['xsd:schema']) return parsedXsd['xsd:schema'];
 ...
 * 노드 속성 가져오기
 */
function getNodeAttribute(node: any, attributeName: string): string | undefined {
  // 속성 접두어 처리
  const attributeKeys = [
    `@_${attributeName}`,
    `@_xs:${attributeName}`,
 ...
 * 가져오기 추출
 */
function extractImports(schemaNode: any, schema: SchemaDocument): void {
 ...
 * 요소 추출
 */
function extractElements(schemaNode: any, schema: SchemaDocument): void {
 ...
 * 복합 타입 추출
 */
function extractComplexTypes(schemaNode: any, schema: SchemaDocument): void {
 ...
 * 복합 타입 내용 추출
 */
function extractComplexTypeContent(complexTypeNode: any): SchemaComplexType {
  ...
  if (sequence) {
    // 임시 스키마 문서 생성
    const tempSchema: SchemaDocument = {
      fileName: '',
      targetNamespace: '',
      imports: [],
      elements: [],
 ...
 * 타입에서 속성 추출
 */
function extractAttributesFromType(typeNode: any, complexType: SchemaComplexType): void {
  ...
  for (const attributeNode of attributes) {
    complexType.attributes.push({
      ...
      use: getNodeAttribute(attributeNode, 'use') as 'optional' | 'required' | 'prohibited',
...
function extractDocumentation(node: any): string | undefined {
  ...
  // 텍스트 노드 또는 CDATA 처리
  if (documentation['#text']) return documentation['#text'];
  if (documentation['#cdata']) return documentation['#cdata'];
 ...
 * 단순 타입 추출
 */
function extractSimpleTypes(schemaNode: any, schema: SchemaDocument): void {
  // 단순 타입 추출 로직 구현...
}
 ...
 * 그룹 추출
 */
function extractGroups(schemaNode: any, schema: SchemaDocument): void {
  // 그룹 추출 로직 구현...
}
 ...
 * 속성 추출
 */
function extractAttributes(schemaNode: any, schema: SchemaDocument): void {
  // 전역 속성 추출 로직 구현...
}
 ...
 * 속성 그룹 추출
 */
function extractAttributeGroups(schemaNode: any, schema: SchemaDocument): void {
  // 속성 그룹 추출 로직 구현...
}
 ...
 * 특정 이름의 노드 찾기
 */
function findNode(parentNode: any, nodeName: string): any {
 ...
 * 특정 이름의 노드 배열 찾기
 */
function findNodes(parentNode: any, nodeName: string): any[] {
