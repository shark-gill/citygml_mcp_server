 ...
 * @param ttl 만료 시간 (밀리초)
 */
export function setMemoryCache<T>(key: string, data: T, ttl: number = DEFAULT_TTL): void {
  memoryCache[key] = {
    data,
 ...
 * @param ttl 만료 시간 (밀리초)
 */
export function setDiskCache<T>(key: string, data: T, ttl: number = DEFAULT_TTL): void {
  initCache();
 ...
 * @returns 캐시된 데이터 또는 undefined
 */
export function getDiskCache<T>(key: string): T | undefined {
  const cacheFile = path.join(CACHE_DIR, sanitizeFilename(key));
