...
import { SchemaDocument, SchemaRegistry } from '../types/schema.js';
 ...
 * @returns 스키마 레지스트리
 */
export async function loadAllSchemas(): Promise<SchemaRegistry> {
  // 메모리 캐시 확인
  const memCache = getMemoryCache<SchemaRegistry>(SCHEMA_REGISTRY_CACHE_KEY);
  if (memCache) {
  ...
  // 디스크 캐시 확인
  const diskCache = getDiskCache<SchemaRegistry>(SCHEMA_REGISTRY_CACHE_KEY);
  if (diskCache) {
 ...
 * @returns 파싱된 스키마 문서
 */
export async function loadSchema(schemaFile: string): Promise<SchemaDocument> {
  const fileName = path.basename(schemaFile);
  const cacheKey = `schema_${fileName}`;
  
  // 메모리 캐시 확인
  const memCache = getMemoryCache<SchemaDocument>(cacheKey);
  if (memCache) {
    return memCache;
  }
  
  // 디스크 캐시 확인
  const diskCache = getDiskCache<SchemaDocument>(cacheKey);
  if (diskCache) {
 ...
 * @returns 파싱된 스키마 문서
 */
export async function loadModuleSchema(moduleName: string): Promise<SchemaDocument | null> {
 ...
 */
function getModuleNameFromNamespace(namespace: string): string | null {
