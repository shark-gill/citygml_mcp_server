 ...
 * @returns 파싱된 인코딩 문서
 */
export async function parseEncodingDocument(): Promise<EncodingDocument> {
  // 메모리 캐시 확인
  const memCache = getMemoryCache<EncodingDocument>(ENCODING_CACHE_KEY);
  if (memCache) {
  ...
  // 디스크 캐시 확인
  const diskCache = getDiskCache<EncodingDocument>(ENCODING_CACHE_KEY);
  if (diskCache) {
  ...
  // 모델 정보 초기화
  const encodingModel: EncodingModel = {
    version,
    encodingType: 'GML',
 ...
 * 네임스페이스 정보 추출
 */
function extractNamespaces($: cheerio.CheerioAPI): Namespace[] {
  ...
  $('table').each((_index: number, table: any) => {
    ...
    const headers = $headerRow.find('th, td').map((_i: number, el: any) => $(el).text().toLowerCase()).get();
    
    const prefixIdx = headers.findIndex((h: string) => 
      h.includes('prefix') || h.includes('namespace') || h.includes('xmlns'));
    
    const uriIdx = headers.findIndex((h: string) => 
      h.includes('uri') || h.includes('url') || h.includes('namespace'));
    ...
    $rows.slice(1).each((_i: number, row: any) => {
 ...
 * 인코딩 규칙 추출
 */
function extractEncodingRules($: cheerio.CheerioAPI): EncodingRule[] {
  const rules: EncodingRule[] = [];
  
  // 'rule', 'encoding', 'requirement' 등의 단어가 있는 제목 찾기
  $('h1, h2, h3, h4, h5').each((_index: number, heading: any) => {
 ...
 * 텍스트에서 클래스 이름 찾기
 */
function findClassesInText(text: string): string[] {
 ...
 * 텍스트에서 예제 코드 참조 찾기
 */
function findExamplesInText(text: string): string[] {
 ...
 * 예제 코드 추출
 */
function extractExamples($: cheerio.CheerioAPI): Example[] {
  const examples: Example[] = [];
  
  // 코드 블록 추출
  $('pre, code, .listing, .example').each((_index: number, element: any) => {
    ...
    // 예제 언어 결정
    let language: 'XML' | 'JSON' | 'SQL' | 'Other' = 'XML';
 ...
 * 문서 섹션 추출
 */
function extractSections($: cheerio.CheerioAPI): EncodingSection[] {
  const sections: EncodingSection[] = [];
  
  // 상위 수준의 헤딩부터 순회
  $('h1, h2').each((_index: number, element: any) => {
    const $heading = $(element);
    const id = $heading.attr('id') || `section-${sections.length + 1}`;
    const title = $heading.text().trim();
    
    // 상위 수준 섹션 생성
    const section: EncodingSection = {
      id,
      title,
 ...
 * 하위 섹션 추출
 */
function extractSubsections($: cheerio.CheerioAPI, $parentHeading: cheerio.Cheerio<any>): EncodingSection[] {
  const subsections: EncodingSection[] = [];
  ...
  if (parentTagName === 'h1') {
    ...
    while ($nextEl.length) {
      ...
      if ($nextEl.is('h2')) {
        ...
        const subsection: EncodingSection = {
          id,
          title,
 ...
 * 요소 내용 추출
 */
function extractElementContent($: cheerio.CheerioAPI, $element: cheerio.Cheerio<any>): string {
 ...
 */
function createDefaultEncodingDocument(): EncodingDocument {
  // 기본 네임스페이스
  const namespaces: Namespace[] = [
    {
      prefix: 'citygml',
  ...
  // 기본 인코딩 규칙
  const encodingRules: EncodingRule[] = [
    {
      id: 'rule-1',
      name: 'GML 인코딩 규칙',
