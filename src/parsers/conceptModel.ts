 ...
 */
export async function parseConceptModel(): Promise<ConceptModelDocument> {
  // 메모리 캐시 확인
  const memCache = getMemoryCache<ConceptModelDocument>(CONCEPT_MODEL_CACHE_KEY);
  if (memCache) {
  ...
  // 디스크 캐시 확인
  const diskCache = getDiskCache<ConceptModelDocument>(CONCEPT_MODEL_CACHE_KEY);
  if (diskCache) {
  ...
  // 결과 문서 생성
  const result: ConceptModelDocument = {
    title,
    version,
 ...
 * 문서에서 버전 정보 추출
 */
function extractVersion($: cheerio.CheerioAPI): string {
 ...
 * 문서에서 섹션 정보 추출
 */
function extractSections($: cheerio.CheerioAPI): ConceptModelSection[] {
  const sections: ConceptModelSection[] = [];
  
  // 상위 수준의 헤딩부터 순회
  $('h1, h2').each((i: number, el: any) => {
    const $el = $(el);
    const id = $el.attr('id') || `section-${i}`;
    const title = $el.text().trim();
    
    // 상위 수준 섹션 생성
    const section: ConceptModelSection = {
      id,
      title,
 ...
 * 하위 섹션 추출
 */
function extractSubsections($: cheerio.CheerioAPI, $parentHeading: cheerio.Cheerio<any>): ConceptModelSection[] {
  const subsections: ConceptModelSection[] = [];
  ...
  if (parentTagName === 'h1') {
    ...
    while ($nextEl.length) {
      ...
      if ($nextEl.is('h2')) {
        ...
        const subsection: ConceptModelSection = {
          id,
          title,
 ...
 * 섹션 내용 추출
 */
function extractSectionContent($: cheerio.CheerioAPI, $heading: cheerio.Cheerio<any>): string {
 ...
 * 관련 클래스 찾기
 */
function findRelatedClasses($: cheerio.CheerioAPI, $heading: cheerio.Cheerio<any>): string[] {
 ...
 * 관련 다이어그램 찾기
 */
function findRelatedDiagrams($: cheerio.CheerioAPI, $heading: cheerio.Cheerio<any>): string[] {
  const relatedDiagrams: string[] = [];
  const content = extractSectionContent($, $heading);
 ...
 * 다이어그램 추출
 */
function extractDiagrams($: cheerio.CheerioAPI): UMLDiagram[] {
  const diagrams: UMLDiagram[] = [];
  
  // 다이어그램(이미지가 있는 figure 요소) 찾기
  $('figure, .figure').each((i: number, el: any) => {
    const $figure = $(el);
    ...
    if ($img.length) {
      ...
      const elements: UMLElement[] = extractUMLElements($, $figure);
 ...
 * UML 요소 추출
 */
function extractUMLElements($: cheerio.CheerioAPI, $figure: cheerio.Cheerio<any>): UMLElement[] {
 ...
 * 모듈 추출
 */
function extractModules($: cheerio.CheerioAPI): Module[] {
  const modules: Module[] = [];
  
  // 모듈 섹션 찾기
  // 모듈 관련 헤딩 찾기
  const moduleHeadings = $('h1, h2, h3').filter((i: number, el: any) => {
  ...
  moduleHeadings.each((i: number, el: any) => {
    const $heading = $(el);
 ...
 * 특정 모듈의 클래스 추출
 */
function extractClassesForModule($: cheerio.CheerioAPI, $moduleHeading: cheerio.Cheerio<any>, moduleName: string): Class[] {
  const classes: Class[] = [];
 ...
 * 클래스 정보 찾기
 */
function findClassInfo($: cheerio.CheerioAPI, className: string): Class | null {
  // 클래스 관련 헤딩 찾기
  const classHeadings = $('h2, h3, h4, h5').filter((i: number, el: any) => {
    const text = $(el).text();
 ...
 * 클래스 속성 추출
 */
function extractAttributes($: cheerio.CheerioAPI, content: string, className: string): Attribute[] {
  ...
  $tables.each((i: number, table: any) => {
    ...
    const headers = $headerRow.find('th').map((j: number, th: any) => $(th).text().trim().toLowerCase()).get();
    
    // 속성 관련 테이블인지 확인 (이름과 타입 컬럼 필요)
    const nameIndex = headers.findIndex((h: string) => h.includes('name') || h.includes('attribute'));
    const typeIndex = headers.findIndex((h: string) => h.includes('type'));
    const cardinalityIndex = headers.findIndex((h: string) => h.includes('cardinality') || h.includes('multiplicity'));
    ...
    // 속성 정보 추출
    $rows.slice(1).each((j: number, row: any) => {
 ...
 * 클래스 관계 추출
 */
function extractAssociations($: cheerio.CheerioAPI, content: string, className: string): Association[] {
