...
import { Context, ContextItem, ContextItemType } from './builder.js';
...
export interface FormattingOptions {
  format: OutputFormat;
  maxDepth?: number;       // 계층적 정보 최대 깊이
  includeContent?: boolean; // 전체 콘텐츠 포함 여부
  maxItems?: number;       // 최대 항목 수
  compactMode?: boolean;   // 간결한 모드
  highlightKeywords?: string[]; // 강조할 키워드
  codeFormatting?: boolean; // 코드 예제 포맷팅 적용
}
...
export class ContextFormatter {
   ...
   * @returns 포맷팅된 문자열
   */
  static format(context: Context, options: FormattingOptions): string {
    // 옵션 기본값 설정
   ...
   * JSON 형식으로 변환
   */
  private static formatJSON(context: Context, items: ContextItem[], options: Required<FormattingOptions>): string {
    // 컨텍스트의 깊은 복사본 생성
    const contextCopy = {
      id: context.id,
      query: context.query,
    ...
    for (const item of items) {
      ...
      if (options.includeContent) {
        (itemCopy as any).content = item.content;
      }
      
      (contextCopy.items as any[]).push(itemCopy);
    }
   ...
   * 마크다운 형식으로 변환
   */
  private static formatMarkdown(context: Context, items: ContextItem[], options: Required<FormattingOptions>): string {
    ...
    for (const [type, typeItems] of Object.entries(groupedItems)) {
      markdown += `### ${this.formatItemTypeName(type as ContextItemType)}\n\n`;
   ...
   * 텍스트 형식으로 변환
   */
  private static formatText(context: Context, items: ContextItem[], options: Required<FormattingOptions>): string {
    ...
    // 그룹별 출력
    for (const [type, typeItems] of Object.entries(groupedItems)) {
      text += `[${this.formatItemTypeName(type as ContextItemType)}]\n\n`;
   ...
   * HTML 형식으로 변환
   */
  private static formatHTML(context: Context, items: ContextItem[], options: Required<FormattingOptions>): string {
...
<body>
  ...
  <h2>항목 목록</h2>`;
    ...
    for (const [type, typeItems] of Object.entries(groupedItems)) {
      html += `
  <h3>${this.formatItemTypeName(type as ContextItemType)}</h3>
   ...
   * 항목 타입별 그룹화
   */
  private static groupItemsByType(items: ContextItem[]): Record<string, ContextItem[]> {
   ...
   * 항목 타입 이름 포맷팅
   */
  private static formatItemTypeName(type: ContextItemType): string {
    switch (type) {
   ...
   * 마크다운에서 항목 콘텐츠 포맷팅
   */
  private static formatItemContentMarkdown(item: ContextItem, options: Required<FormattingOptions>): string {
    let markdown = '';
   ...
   * 텍스트에서 항목 콘텐츠 포맷팅
   */
  private static formatItemContentText(item: ContextItem, options: Required<FormattingOptions>): string {
    let text = '';
   ...
   * HTML에서 항목 콘텐츠 포맷팅
   */
  private static formatItemContentHTML(item: ContextItem, options: Required<FormattingOptions>): string {
    let html = '';
   ...
   * @returns 요약 문자열
   */
  static generateSummary(context: Context, maxLength: number = 200): string {
   ...
   * 항목 타입 목록 포맷팅
   */
  private static formatItemTypeList(types: ContextItemType[]): string {
