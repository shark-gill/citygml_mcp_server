...
import { MODULE_NAMESPACES } from '../config.js';
...
// 추가로 config.ts의 MODULE_NAMESPACES에서 누락된 네임스페이스 가져오기
Object.entries(MODULE_NAMESPACES).forEach(([key, value]) => {
  if (!namespaces[key as keyof typeof namespaces]) {
    (namespaces as Record<string, string>)[key] = value;
  }
});
...
export class ConceptExtractor {
  ...
  private conceptModel: ConceptualModel;
   ...
   */
  public async extractAllModules(): Promise<ConceptualModel> {
    try {
      ...
      const imports = select('//xs:import', mainXsdDoc) as Element[];
   ...
   * 개별 모듈 XSD 파일 처리
   */
  private async processModuleFile(moduleName: string, schemaLocation: string): Promise<void> {
    try {
      ...
      // 네임스페이스 정보 추출
      const schemaElements = select('//xs:schema', moduleXsdDoc) as Node[];
   ...
   * 모듈에서 클래스 정보 추출
   */
  private extractClasses(doc: Document, moduleName: string): Class[] {
    const classes: Class[] = [];
    
    // complexType 요소 추출 (클래스에 해당)
    const complexTypes = select('//xs:complexType', doc) as Element[];
   ...
   * 모듈에서 코드 리스트 추출
   */
  private extractCodelists(doc: Document, moduleName: string): Codelist[] {
    const codelists: Codelist[] = [];
    
    // simpleType에서 enumeration이 있는 요소 추출
    const simpleTypes = select('//xs:simpleType[.//xs:enumeration]', doc) as Element[];
    
    for (const simpleType of simpleTypes) {
      const name = simpleType.getAttribute('name');
      if (name && !name.endsWith('EnumBase')) {
        const values = select('.//xs:enumeration', simpleType) as Element[];
        
        const codelistInfo: Codelist = {
          name,
   ...
   * 모듈에서 열거형 추출
   */
  private extractEnumerations(doc: Document, moduleName: string): Enumeration[] {
    const enumerations: Enumeration[] = [];
    
    // simpleType에서 enumeration이 있고 이름이 EnumBase로 끝나는 요소 추출
    const simpleTypes = select('//xs:simpleType[.//xs:enumeration and contains(@name, "EnumBase")]', doc) as Element[];
   ...
   * 모듈 의존성 추출
   */
  private extractDependencies(doc: Document): string[] {
    const dependencies: string[] = [];
    
    // 모든 import 요소에서 의존성 추출
    const imports = select('//xs:import', doc) as Element[];
   ...
   * 클래스에서 속성 추출
   */
  private extractAttributes(complexType: Element): Attribute[] {
    const attributes: Attribute[] = [];
    
    // 요소 추출
    const elements = select('.//xs:element', complexType) as Element[];
    ...
    // 속성 추출
    const attrElements = select('.//xs:attribute', complexType) as Element[];
   ...
   * 클래스에서 연관관계 추출
   */
  private extractAssociations(complexType: Element): Association[] {
    const associations: Association[] = [];
    
    // 참조 요소(ref 속성이 있는)를 통한 연관관계 추출
    const refElements = select('.//xs:element[@ref]', complexType) as Element[];
   ...
   * 요소에서 제약조건 추출
   */
  private extractConstraints(complexType: Element): Constraint[] {
    const constraints: Constraint[] = [];
    
    // 주석에서 제약조건 추출
    // 일반적으로 주석에 제약조건이 텍스트로 포함됨
    const annotations = select('.//xs:annotation', complexType) as Element[];
    
    for (const annotation of annotations) {
      const documentation = select('.//xs:documentation', annotation) as Element[];
   ...
   * 클래스의 상위 클래스 추출
   */
  private extractSuperClasses(complexType: Element): string[] {
    const superClasses: string[] = [];
    
    // extension 요소 찾기
    const extensions = select('.//xs:extension', complexType) as Element[];
   ...
   * 모듈 설명 추출
   */
  private extractModuleDescription(doc: Document): string {
    // 모듈 설명은 주로 최상위 주석에 있음
    const annotations = select('//xs:annotation', doc) as Element[];
    
    if (annotations && annotations.length > 0) {
      const documentation = select('.//xs:documentation', annotations[0]) as Element[];
   ...
   * 요소에서 문서화(설명) 추출
   */
  private extractDocumentation(element: Element): string {
    const annotations = select('.//xs:annotation', element) as Element[];
   ...
   * 개념 모델과 문서 섹션 매핑
   */
  public mapConceptsToDocSections(docSectionMap: Record<string, string>): Map<string, ConceptModelSection> {
    const conceptSectionMap = new Map<string, ConceptModelSection>();
    
    // 각 모듈과 클래스에 대한 문서 섹션 매핑
    for (const module of this.conceptModel.modules) {
      for (const classInfo of module.classes) {
        // 클래스 이름으로 문서 섹션 찾기
        const sectionKey = Object.keys(docSectionMap).find(key => 
          classInfo.name.includes(key) || key.includes(classInfo.name)
        );
        
        if (sectionKey) {
          const sectionInfo: ConceptModelSection = {
      ...
      if (module.codelists) {
        for (const codelist of module.codelists) {
          const sectionKey = Object.keys(docSectionMap).find(key => 
            codelist.name.includes(key) || key.includes(codelist.name)
          );
      ...
      if (module.enumerations) {
        for (const enumeration of module.enumerations) {
          const sectionKey = Object.keys(docSectionMap).find(key => 
            enumeration.name.includes(key) || key.includes(enumeration.name)
          );
