...
function isNodeArray(obj: any): obj is Node[] {
...
interface LodInfo {
  maxLod: number;
  minLod: number;
  lodAttributes: LodAttribute[];
}

// LOD 속성 인터페이스
interface LodAttribute {
  name: string;
  level: number;
  type: string;
  role?: string;
...
// 기하 표현 정보 인터페이스
interface GeometryRepresentation {
  type: string;
  lodLevel: number;
  multiplicity: string;
  dimension?: number;
...
export class ObjectExtractor {
   ...
   */
  public extractAllCityObjects(): Map<string, CityObject> {
    if (!this.conceptualModel) {
      throw new Error('개념 모델이 설정되지 않았습니다. setConceptualModel()을 먼저 호출하세요.');
    }

    const cityObjects = new Map<string, CityObject>();
   ...
   * 특정 모듈에서 City 객체 추출
   */
  private extractCityObjectsFromModule(module: Module): Map<string, CityObject> {
    const objects = new Map<string, CityObject>();
   ...
   */
  private isCityObjectClass(classInfo: Class): boolean {
    ...
    if (classInfo.superClasses) {
      for (const superClass of classInfo.superClasses) {
        if (
          superClass.includes('AbstractCityObject') || 
   ...
   * 기하 속성 추출
   */
  private extractGeometryProperties(classInfo: Class): any[] {
    const geometryProperties = [];
    
    // 속성에서 기하 속성 추출
    for (const attribute of classInfo.attributes) {
      const isGeometryAttribute = this.geometryClassPatterns.some(pattern => 
        attribute.type.includes(pattern)
      );
    ...
    if (classInfo.associations) {
      for (const association of classInfo.associations) {
        const isGeometryAssociation = this.geometryClassPatterns.some(pattern => 
          association.target.includes(pattern)
        );
   ...
   * LOD 정보 추출
   */
  private extractLodInfo(classInfo: Class): LodInfo {
   ...
   */
  private extractLodLevel(name: string): number {
   ...
   */
  private extractThematicAttributes(classInfo: Class): any[] {
   ...
   */
  private buildObjectHierarchy(objects: Map<string, CityObject>): void {
   ...
   * 모듈별 객체 분류
   */
  public classifyObjectsByModule(): Record<string, string[]> {
    if (!this.conceptualModel) {
      throw new Error('개념 모델이 설정되지 않았습니다. setConceptualModel()을 먼저 호출하세요.');
    }

    const objectsByModule: Record<string, string[]> = {};
   ...
   */
  public extractGeometryRepresentations(cityObjectName: string): GeometryRepresentation[] {
    if (!this.conceptualModel) {
      throw new Error('개념 모델이 설정되지 않았습니다. setConceptualModel()을 먼저 호출하세요.');
    }
    ...
    const geometryRepresentations: GeometryRepresentation[] = [];
   ...
   * 기하 타입에서 차원 추론
   */
  private inferGeometryDimension(geometryType: string): number {
   ...
   */
  public async processXsdFile(xsdFilePath: string): Promise<void> {
    try {
      ...
      // 스키마 요소 찾기
      const schemaElements = xpath.select('//xs:schema', xsdDoc) as Node[];
      ...
      // 복합 타입 요소 추출
      const complexTypeElements = xpath.select('//xs:complexType', xsdDoc) as Node[];
   ...
   */
  private isCityObjectTypeByName(typeName: string): boolean {
