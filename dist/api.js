...
import { loadAllSchemas } from "./parsers/schema.js";
...
function registerTools(server) {
    // 개념 모델 쿼리 도구
    server.tool("query_concept_model", {
        ...
        try {
            ...
            for (const mod of targetModules) {
                ...
                if (!entity_type || entity_type === "class") {
                    const matchingClasses = mod.classes.filter(cls => cls.name.toLowerCase().includes(query.toLowerCase()) ||
                ...
                if (!entity_type || entity_type === "attribute") {
                    for (const cls of mod.classes) {
                        const matchingAttrs = cls.attributes.filter(attr => attr.name.toLowerCase().includes(query.toLowerCase()) ||
                ...
                if (!entity_type || entity_type === "association") {
                    for (const cls of mod.classes) {
                        if (!cls.associations)
                            continue;
                        const matchingAssocs = cls.associations.filter(assoc => assoc.name.toLowerCase().includes(query.toLowerCase()) ||
                    ...
                    if (typeName.toLowerCase().includes(query.toLowerCase())) {
                        ...
                        for (const type of types) {
                            ...
                            for (const ns in registry.schemas) {
                                if (registry.schemas[ns].some(s => s.complexTypes.some(t => t.name === typeName) ||
                            ...
                            if (targetNamespaces.includes(namespace) || targetNamespaces.length === 0) {
                                ...
                                results.push({
                                    ...
                                    name: typeName,
                                    isAbstract: isComplexType ? type.abstract || false : false,
                                    base: isComplexType ? type.base : undefined,
                                    namespace,
    ...
    // 인코딩 예제 검색 도구
    server.tool("find_encoding_examples", {
        ...
        try {
            ...
            const examples = encodingDoc.model.examples.filter(example => example.relatedClasses.includes(class_name) &&
            ...
            // 결과 포맷팅
            const resultText = examples.map(example => (`# ${example.title}\n` +
