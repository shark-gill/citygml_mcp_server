...
(function (TemplateType) {
    TemplateType["CLASS_DETAIL"] = "class_detail";
    TemplateType["MODULE_SUMMARY"] = "module_summary";
    TemplateType["CLASS_HIERARCHY"] = "class_hierarchy";
    TemplateType["ELEMENT_DETAIL"] = "element_detail";
    TemplateType["ENCODING_GUIDE"] = "encoding_guide";
    TemplateType["COMPARE_CLASSES"] = "compare_classes";
    TemplateType["SCHEMA_MAPPING"] = "schema_mapping";
    TemplateType["FEATURE_OVERVIEW"] = "feature_overview";
...
export class TemplateManager {
     ...
     * @returns 렌더링된 문자열
     */
    static applyTemplate(templateType, variables, outputFormat = OutputFormat.MARKDOWN) {
        // 템플릿 찾기
     ...
     * @returns 렌더링된 문자열
     */
    static applyTemplateToContext(context, templateType, additionalVars = {}, outputFormat = OutputFormat.MARKDOWN) {
        // 컨텍스트에서 변수 추출
        const contextVars = {
     ...
     * @returns 컨텍스트 객체
     */
    static createTemplateContext(templateType, conceptModel, encodingDoc, schemaRegistry, params = {}) {
        // 템플릿 유형에 맞는 컨텍스트 빌더 초기화
        const builder = new ContextBuilder(`template_${templateType}_${Date.now()}`);
        // 데이터 소스 설정
        if (conceptModel)
            builder.setConceptModel(conceptModel);
        if (encodingDoc)
            builder.setEncodingDocument(encodingDoc);
        if (schemaRegistry)
     ...
     * @returns 처리된 문자열
     */
    static processHelperFunctions(template, variables) {
        let result = template;
        // capitalize 함수
