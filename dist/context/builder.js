...
(function (ContextItemType) {
    ContextItemType["MODULE"] = "module";
    ContextItemType["CLASS"] = "class";
    ContextItemType["ATTRIBUTE"] = "attribute";
    ContextItemType["ASSOCIATION"] = "association";
    ContextItemType["EXAMPLE"] = "example";
    ContextItemType["SCHEMA"] = "schema";
    ContextItemType["ELEMENT"] = "element";
    ContextItemType["TYPE"] = "type";
    ContextItemType["ENCODING_RULE"] = "encoding_rule";
...
export class ContextBuilder {
     ...
     * 컨텍스트 빌더 생성자
     */
    constructor(contextId = `ctx_${Date.now()}`) {
        // 새 컨텍스트 초기화
        this.context = {
            id: contextId,
            timestamp: Date.now(),
     ...
     */
    analyzeConceptModel(query, threshold) {
        if (!this.conceptModel)
            return;
     ...
     * @param threshold 관련성 임계값
     */
    analyzeEncodingDoc(query, threshold) {
        if (!this.encodingDoc)
            return;
        // 인코딩 규칙 분석
     ...
     * @param threshold 관련성 임계값
     */
    analyzeSchemaRegistry(query, threshold) {
        if (!this.schemaRegistry)
            return;
        // 요소 분석
     ...
     */
    addModuleToContext(module, relevance) {
     ...
     * @param relevance 관련성 점수
     */
    addClassToContext(cls, module, relevance) {
     ...
     * @param relevance 관련성 점수
     */
    addAttributeToContext(attr, cls, module, relevance) {
     ...
     * @param relevance 관련성 점수
     */
    addAssociationToContext(assoc, cls, module, relevance) {
     ...
     * @param relevance 관련성
     */
    addRuleToContext(rule, relevance) {
     ...
     * @param relevance 관련성
     */
    addExampleToContext(example, relevance) {
     ...
     * @param relevance 관련성 점수
     */
    addElementToContext(element, elementName, relevance) {
     ...
     * @param relevance 관련성 점수
     */
    addTypeToContext(type, typeName, relevance) {
        const typeId = `type_${typeName.toLowerCase()}`;
        // 이미 추가된 항목인지 확인
        ...
        if (isComplex && type.base !== undefined && type.base) {
            const baseName = (type.base?.split(':').pop()) || '';
